...
app/
  api/             → route handlers (thin — logic goes to lib/)
    admin/         → protected admin routes (15 sub-routes)
      ...
      hotmart/         → Hotmart admin endpoints
        ...
        credentials/   → Hotmart API credentials (GET state + PUT save, DB-backed)
  ...
  components/
    ...
    admin/         → admin area components
      ...
      hotmart/       → HotmartTab (product ID, credentials, webhook, plan sync + quota editing)
...
lib/
  ...
  settings.ts      → database-backed configuration + secret encryption helpers
    Key functions: `getSetting`, `getSettingOrEnv`, `upsertSetting`, `getSecretSetting`, `upsertSecretSetting`, `hasSecretSetting` (checks encrypted secret existence without decrypting)
...
### Hotmart Credentials — DB-backed storage

Hotmart API credentials are stored in the `Setting` table (encrypted where sensitive) and managed via the admin panel.

- **API Route**: `app/api/admin/hotmart/credentials/route.ts`
  - `GET` — returns credentials state: `clientId` (plaintext), `hasClientSecret` (bool), `hasBasicToken` (bool), `hasWebhookSecret` (bool), `sandbox` (bool). Secrets are never returned in plaintext — only existence is checked via `hasSecretSetting()`.
  - `PUT` — saves credentials: `clientId` stored as plaintext setting; `clientSecret`, `basicToken`, `webhookSecret` stored encrypted via `upsertSecretSetting()` (AES-256-GCM). Empty string submissions are ignored — a field is only updated when a non-empty value is provided.
- **Credential resolution** (`lib/hotmart/config.ts` → `getHotmartConfig()`):
  - `clientId` — DB setting first (`hotmart.client_id`), fallback to `HOTMART_CLIENTE_ID` env var
  - `clientSecret` — DB encrypted setting first (`hotmart.client_secret`), fallback to `HOTMART_CLIENT_SECRET` env var
  - `basicToken` — DB encrypted setting first (`hotmart.basic_token`), fallback to `HOTMART_BASIC` env var
  - `sandbox` — DB setting (`hotmart.sandbox`), readable as bool
  - **Webhook secret exception**: `verifySignature()` in `lib/hotmart/webhook.ts` still reads from `HOTMART_WEBHOOK_SECRET` env var only — NOT from the DB. The credentials route accepts and saves it to the DB for future use, but validation still uses the env var.
- **Setting keys** (from `SETTING_KEYS` in `lib/settings.ts`):
  - `hotmart.product_id`, `hotmart.client_id`, `hotmart.client_secret`, `hotmart.basic_token`, `hotmart.webhook_secret`, `hotmart.sandbox`
- **Admin UI**: `CredentialsCard` inside `HotmartTab`
  - 4 fields: Client ID, Client Secret, Basic Token, Webhook Secret
  - Secret fields (Client Secret, Basic Token, Webhook Secret) use a masking UX: display `••••••••••••` when a value is already saved; clear on focus so the user can type a new value; restore mask on blur if left empty (sentinel value `__SAVED__` used internally)
  - No sandbox toggle in the UI — the sandbox setting can be managed programmatically
- Do not hardcode Hotmart credentials in the codebase — always use `getHotmartConfig()` or the individual `getSetting`/`getSecretSetting` helpers
...
- Displays subscription metrics and subscriber table.
- **PlansCard is NOT on this page** — plans management (sync, quota editing, visibility toggle) lives exclusively in the config page → HotmartTab.
- Do not re-add a PlansCard or plan management widget to the admin dashboard page.

### HotmartTab (`app/components/admin/hotmart/HotmartTab.tsx`)

- `CredentialsCard` — manages Hotmart API credentials stored in the DB; see "Hotmart Credentials" section.
- `PlansCard` — plan sync and quota editor with per-plan `showOnLanding` visibility toggle.
  - The `showOnLanding` toggle uses optimistic local state (`localVisible` via `useState`) synced by a `useEffect` when the prop changes. This ensures instant visual feedback without waiting for the server response.
  - Do not revert to prop-only toggle state — the optimistic pattern is intentional.
...
- **`SectionShell`** — wraps all landing sections with background, overlays, glow effects.
  - Accepts a `backgroundSlot?: ReactNode` prop that renders inside an `position: absolute; inset: 0` container, guaranteeing content fills the full section height regardless of flex layout. Use this for full-bleed backgrounds (videos, images) and their overlays.
  - Do not pass full-bleed background elements as regular children — they will not cover the full section.
- **`HeroSection`** — passes `HeroBackgroundVideo` and `HeroOverlays` (contrast, accent, vignette) via `backgroundSlot`. Hero content (text, CTA) is in regular children.
- **ImageSlot** — placeholder component; only use when a real image/media `src` will eventually replace it. Do not use ImageSlot as permanent empty-state decoration in production sections.
- **CTA behavior**: "Quero acesso agora" buttons in `HeroSection` and `HowItWorksSection` scroll to `#planos` (smooth). Buttons inside `PricingSection` plan cards go to the purchase/login flow. Do not change CTA targets without updating all occurrences.
- **Login page** — "Não tem uma assinatura? Assinar" links to `/#planos` (redirects to landing and scrolls to plans section).
- **`LandingFooter`** — uses `<BrandLogo size="sm" href="/" />`. Do not replace it with hardcoded logo markup.
...
- Do not hardcode Hotmart credentials in the codebase — always use `getHotmartConfig()` or the individual `getSetting`/`getSecretSetting` helpers from `lib/settings.ts`.
- Do not pass full-bleed background elements (videos, images, overlays) as regular children to `SectionShell` — use the `backgroundSlot` prop so they correctly fill the full section height.
