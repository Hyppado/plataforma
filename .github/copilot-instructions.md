...
- Plans sync: admin triggers via "Sincronizar" button → fetches plans from Hotmart API → upserts local Plan records (creates with default quotas, updates metadata without touching quotas)
...
- **620 unit tests** (vitest, node env, `__tests__/lib/`, `__tests__/api/`, etc.)
...
- Run `npm run test:all` and confirm that all tests pass (currently 683+).
