...
export async function syncProductRanklist(
  ...
  // "ainda não coletado", não como erro). Um ciclo que não trouxe nada agora
  // mantém o que já tinha, em vez de zerar a tela correspondente.
  let pruned = 0;
  for (const { rankingCycle, rankField } of refreshed) {
    const result = await prisma.echotikProductTrendDaily.deleteMany({
      where: {
        country: region,
        rankingCycle,
        rankField,
  ...
  if (pruned > 0) {
    log.info("Pruned stale product rows", {
      region,
      pruned,
