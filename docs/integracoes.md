...
### Influencer IA (gpt-image-1)

`lib/influencer-ia/generate.ts` → OpenAI Images API (`gpt-image-1`).

Gera imagem UGC ultra-realista combinando imagem de avatar + imagem de produto. O prompt é construído dinamicamente com base em pose, ambiente, estilo, melhorias e categoria do produto (com instruções category-aware em inglês). O resultado é enviado ao Vercel Blob e a URL retornada ao browser para download imediato. Não requer quota — acesso universal a usuários autenticados.

Endpoint: `POST /api/influencer-ia/generate`
